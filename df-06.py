...
#test_buk/flights/airports
def addtimezone(lat, lon):
    try:
        import timezonefinder
        tf = timezonefinder.TimezoneFinder() #주어진 좌표에 해당하는 시간대를 조회할 수 있습니다.
...
def tz_correct(line, airport_timezones_dict):
    ...
    if fields[0] != 'FL_DATE' and len(fields) == 27:
        ...
        for f in [17, 18, 20, 21]: # wheelsoff, wheelson, crsarrtime, arrtime
            fields[f] = add_24h_if_before(fields[f], fields[14]) #인자와 출발 시간을 넘겨줌
...
def run(project, bucket, dataset):
    ...
    schema = 'FL_DATE:date,UNIQUE_CARRIER:string,AIRLINE_ID:string,CARRIER:string,FL_NUM:string,ORIGIN_AIRPORT_ID:string,ORIGIN_AIRPORT_SEQ_ID:integer,' \
             'ORIGIN_CITY_MARKET_ID:string,ORIGIN:string,DEST_AIRPORT_ID:string,DEST_AIRPORT_SEQ_ID:integer,DEST_CITY_MARKET_ID:string,DEST:string,CRS_DEP_TIME:timestamp,DEP_TIME:timestamp,' \
             'DEP_DELAY:float,TAXI_OUT:float,WHEELS_OFF:timestamp,WHEELS_ON:timestamp,TAXI_IN:float,CRS_ARR_TIME:timestamp,ARR_TIME:timestamp,ARR_DELAY:float,CANCELLED:string,CANCELLATION_CODE:string,' \
             'DIVERTED:string,DISTANCE:float,DEP_AIRPORT_LAT:float,DEP_AIRPORT_LON:float,DEP_AIRPORT_TZOFFSET:float,ARR_AIRPORT_LAT:float,ARR_AIRPORT_LON:float,ARR_AIRPORT_TZOFFSET:float,EVENT:string,' \
             'NOTIFY_TIME:timestamp,EVENT_DATA:string'
    (events
